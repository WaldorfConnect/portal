</div>
</main>

<footer>
    <div class="container">
        <div class="row">
            <div class="col-md">
                <p class="mb-2">Ein Dienst der</p>
                <span>
                    <b>Arbeitsgruppe<br>WaldorfConnect</b><br>
                </span>
                <p>&copy; 2024</p>
            </div>
            <div class="col-md">
                <h1 class="h5">Offizielles</h1>
                <ul class="list-unstyled">
                    <li>
                        <a href="//waldorfconnect.de/impressum" target="_blank">Impressum</a>
                    </li>
                    <li>
                        <a href="//waldorfconnect.de/datenschutz" target="_blank">Datenschutz</a>
                    </li>
                </ul>
            </div>
            <div class="col-md">
                <h1 class="h5">Partner & Unterstützung</h1>
                <ul class="list-unstyled">
                    <li>
                        <a href="//waldorfschule.de" target="_blank">Bund der Freien Waldorfschulen e.V.</a>
                    </li>
                    <li>
                        <a href="//waldorfsv.de" target="_blank">WaldorfSV</a>
                    </li>
                    <li>
                        <a href="//elektronisch.dev" target="_blank">elektronisch.dev</a>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</footer>

<script src="<?= base_url('/') ?>/assets/js/lightbox.min.js"></script>
</body>

</html>